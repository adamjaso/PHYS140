function energies=finite(V0,a)
%finite.m
%Author:    Adam J. Jaso, Jr.
%Date:      12/05/2011
%Funcion:   Calculation of the Energies of the Finite Square Well
...
%   SEARCH FUNCTION
function f=f(x,K,a)
    f=sqrt(K)*(sqrt(1-x)*tan(sqrt(1-x)*a) - sqrt(x));
end
...
function wavfunc=wavf(x,el,kappa,a)
    if x < -a
        wavfunc=normconst(x,el,kappa,a) * exp(kappa * a);
    elseif x >= -a && x <= a
